...
local API_BASE = "https://kai-hubjiaoben.vercel.app/api";
...
-- 本地存储
local LOCAL_KEY_FILE = "KaiHub_device_key.txt";
local savedKey = nil;
pcall(function()
	savedKey = readfile(LOCAL_KEY_FILE);
end);

-- 创建卡密验证UI
local KeyFrame = Instance.new("Frame");
KeyFrame.Name = "KeyFrame";
KeyFrame.Size = UDim2.new(0, (isMobile and 280) or 340, 0, 220);
KeyFrame.Position = UDim2.new(0.5, -((isMobile and 140) or 170), 0.5, -110);
...
KeyFrameStroke.Transparency = 0.5;
...
KeyTitle.Text = "KaiHub VIP Verify";
...
KeyInput.Size = UDim2.new(0, (isMobile and 240) or 300, 0, 40);
KeyInput.Position = UDim2.new(0.5, -((isMobile and 120) or 150), 0, 70);
...
KeyStatus.Position = UDim2.new(0, 0, 0, 118);
KeyStatus.BackgroundTransparency = 1;
KeyStatus.Text = "";
KeyStatus.TextColor3 = THEME.Error;
...
VerifyBtn.Size = UDim2.new(0, (isMobile and 240) or 300, 0, 42);
VerifyBtn.Position = UDim2.new(0.5, -((isMobile and 120) or 150), 0, 150);
...
local function verifyKey(key, callback)
	local ok, result = pcall(function()
		local url = API_BASE .. "/check?key=" .. HttpService:UrlEncode(key);
	...
	if ok and result then
		...
		callback({valid = false, message = "Network error, try again"});
	end;
end;

-- 使用卡密函数
local function useKey(key, callback)
	local ok, result = pcall(function()
		local url = API_BASE .. "/use?key=" .. HttpService:UrlEncode(key) .. "&device=" .. HttpService:UrlEncode(deviceId) .. "&player=" .. HttpService:UrlEncode(LocalPlayer.Name);
		local response = game:HttpGet(url);
		return HttpService:JSONDecode(response);
	end);
	if ok and result then
		...
		callback({success = false, message = "Network error, try again"});
	end;
end;

-- 显示成功界面
local function showKeySuccess()
	KeyTitle.Text = "VERIFIED";
	KeyTitle.TextColor3 = THEME.Success;
	KeyInput.Visible = false;
	VerifyBtn.Visible = false;
	KeyStatus.Text = "Welcome to KaiHub VIP!";
	...
	task.delay(1.5, function()
		...
		TweenService:Create(KeyStatus, TweenInfo.new(0.3), {TextTransparency = 1}):Play();
		task.delay(0.5, function()
			KeyFrame.Visible = false;
			showStartButtonReal();
		end);
...
VerifyBtn.MouseButton1Click:Connect(function()
	...
	verifyKey(key, function(result)
		if result.used then
			-- 卡密已被使用
			if result.device == deviceId then
				-- 是当前设备，自动通过
				KeyStatus.Text = "Device verified!";
				KeyStatus.TextColor3 = THEME.Success;
				pcall(function() writefile(LOCAL_KEY_FILE, key) end);
				showKeySuccess();
			else
				-- 不是当前设备，踢出
				KeyStatus.Text = KICK_MSG;
		...
		elseif result.valid then
			-- 卡密有效，绑定设备
			useKey(key, function(useResult)
				if useResult.success then
					KeyStatus.Text = "Key activated!";
					KeyStatus.TextColor3 = THEME.Success;
					pcall(function() writefile(LOCAL_KEY_FILE, key) end);
					showKeySuccess();
				else
					KeyStatus.Text = useResult.message or "Activation failed";
					KeyStatus.TextColor3 = THEME.Error;
					VerifyBtn.Text = "VERIFY KEY";
					VerifyBtn.BackgroundColor3 = THEME.Accent;
				end;
			...
			-- 卡密无效
			KeyStatus.Text = result.message or "Invalid key!";
			KeyStatus.TextColor3 = THEME.Error;
			VerifyBtn.Text = "VERIFY KEY";
			VerifyBtn.BackgroundColor3 = THEME.Accent;
		end;
...
-- 真正的显示启动按钮函数
local function showStartButtonReal()
	pulseRunning = false;
...
local function checkKeyVerification()
	if savedKey and #savedKey > 0 then
		-- 有保存的卡密，验证是否有效
		verifyKey(savedKey, function(result)
			if result.used and result.device == deviceId then
				-- 已绑定当前设备，直接显示启动按钮
				showStartButtonReal();
			else
				-- 需要重新验证
				savedKey = nil;
				pcall(function() delfile(LOCAL_KEY_FILE) end);
				showKeyVerify();
			end;
		end);
	else
...
local function showStartButton()
	-- 加载完成后检查卡密
	task.delay(0.5, checkKeyVerification);
end;
