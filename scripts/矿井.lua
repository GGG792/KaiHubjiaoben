...
TabSection = Window:Section({Title="矿井",Opened=true});
local Tab = TabSection:Tab({Title="主要功能",Icon="check"});
Window:SelectTab(1);
Tab:Button({Title="最大力量",Image="check",Callback=function(state)
	WindUI:Notify({Title="塔菲喵",Content="已开启!(没有文字显示但是正常使用)",Duration=3,Icon="bird"});
	local old;
...
local shou = false;
Tab:Toggle({Title="自动收集矿物",Default=false,Image="check",Callback=function(state)
	WindUI:Notify({Title="塔菲喵",Content="请进入洞穴中以便自动拾取矿物",Duration=3,Icon="bird"});
	shou = state;
	while shou and wait() do
...
local sho = false;
Tab:Toggle({Title="自动售卖",Default=false,Image="check",Callback=function(state)
	sho = state;
	while sho and wait() do
