...
local Tab = Window:Tab({Title="功能",Icon="sparkles"});
_G.AutoFarm = false;
Tab:Toggle({Title="自动拾取",Desc="自动收集 Loot 文件夹中的物品",Type="Checkbox",Value=_G.AutoFarm,Callback=function(state)
	_G.AutoFarm = state;
end});
task.spawn(function()
		...
		if _G.AutoFarm then
			...
			if lootFolder then
				for _, item in pairs(lootFolder:GetChildren()) do
					...
					if root then
						local target = (item:IsA("Model") and item:GetPivot()) or (item:IsA("BasePart") and item.CFrame);
						if target then
							root.CFrame = target * CFrame.new(0, 2, 0);
							task.wait(0.2);
						end
					end
				end
