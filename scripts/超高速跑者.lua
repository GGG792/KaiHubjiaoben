...
CombatTab:Toggle({Title="自动刷钱",Desc="自动刷钱",Value=false,Callback=function(State)
	_G.AutoMoney = State;
	if State then
		task.spawn(function()
...
CombatTab:Toggle({Title="自动刷经验",Desc="自动获得经验值",Value=false,Callback=function(State)
	_G.AutoExp = State;
	if State then
		task.spawn(function()
			while _G.AutoExp do
				StepTakenEvent:FireServer(math.huge, false);
				task.wait(9.999999974752427E-07);
			end
		end);
		WindUI:Notify({Title="功能启动",Content="已开始极速刷经验",Duration=2});
	else
		WindUI:Notify({Title="功能关闭",Content="已停止自动刷经验",Duration=2});
	end
end});
