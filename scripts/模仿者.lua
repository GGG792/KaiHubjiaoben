...
local Tabs = {Main=Window:Section({Title="所有关卡",Opened=true})};
local TabHandles = {Q=Tabs.Main:Tab({Title="传送嫉妒1",Icon="layout-grid"}),W=Tabs.Main:Tab({Title="透视功能",Icon="layout-grid"})};
local Button = TabHandles.Q:Button({Title="提示",Desc="第一关卡在日本屋子里",Image="palette",ImageSize=20,Color="White"});
Button = TabHandles.Q:Button({Title="隐身怪物看不见",Desc="有些关卡，一卡一卡的",Locked=false,Callback=function()
	loadstring(game:HttpGet("https://raw.githubusercontent.com/Yungengxin/roblox/main/yinshen"))();
	WindUI:Notify({Title="通知",Content="加载成功",Duration=1,Icon="layout-grid"});
end});
Button = TabHandles.Q:Button({Title="动画房间",Desc="",Locked=false,Callback=function()
	game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-1675.27, -23.32, -3411.01);
	WindUI:Notify({Title="通知",Content="加载成功",Duration=1,Icon="layout-grid"});
end});
Button = TabHandles.Q:Button({Title="放毒老鼠地方",Desc="",Locked=false,Callback=function()
	game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-1562.39, -29.25, -3403.67);
	WindUI:Notify({Title="通知",Content="加载成功",Duration=1,Icon="layout-grid"});
end});
Button = TabHandles.Q:Button({Title="老鼠1",Desc="",Locked=false,Callback=function()
	game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-1524.21, -29.25, -3580.63);
	WindUI:Notify({Title="通知",Content="加载成功",Duration=1,Icon="layout-grid"});
end});
Button = TabHandles.Q:Button({Title="老鼠2",Desc="",Locked=false,Callback=function()
	game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-1642.97, -23.44, -3434.15);
	WindUI:Notify({Title="通知",Content="加载成功",Duration=1,Icon="layout-grid"});
end});
Button = TabHandles.Q:Button({Title="老鼠3",Desc="",Locked=false,Callback=function()
	game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-1680.65, -23.47, -3391.97);
	WindUI:Notify({Title="通知",Content="加载成功",Duration=1,Icon="layout-grid"});
end});
Button = TabHandles.Q:Button({Title="老鼠4",Desc="",Locked=false,Callback=function()
	game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-1620.64, -23.44, -3397.37);
	WindUI:Notify({Title="通知",Content="加载成功",Duration=1,Icon="layout-grid"});
end});
Button = TabHandles.Q:Button({Title="毒老鼠老井",Desc="老鼠毒放在老鼠身上的",Locked=false,Callback=function()
	game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-1531.34, -30.17, -3541.97);
	WindUI:Notify({Title="通知",Content="加载成功",Duration=1,Icon="layout-grid"});
end});
local Button = TabHandles.Q:Button({Title="提示",Desc="第二关卡山坡上的巨蛇",Image="palette",ImageSize=20,Color="White"});
Button = TabHandles.Q:Button({Title="巨大蛇怪山坡秒过",Desc="",Locked=false,Callback=function()
	game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(578.31, 567.98, -380.59);
	WindUI:Notify({Title="通知",Content="加载成功",Duration=1,Icon="layout-grid"});
end});
Button = TabHandles.Q:Button({Title="洞穴秒过",Desc="",Locked=false,Callback=function()
	game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(3837.46, 137.13, 12.84);
	WindUI:Notify({Title="通知",Content="加载成功",Duration=3,Icon="layout-grid"});
end});
local Button = TabHandles.Q:Button({Title="提示",Desc="第三关卡你在村庄里",Image="palette",ImageSize=20,Color="White"});
Button = TabHandles.Q:Button({Title="传送谈话村民",Desc="",Locked=false,Callback=function()
	game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new();
	WindUI:Notify({Title="通知",Content="加载成功",Duration=1,Icon="layout-grid"});
end});
Button = TabHandles.Q:Button({Title="传送洞穴里",Desc="",Locked=false,Callback=function()
...
local function createESP(player)
	...
	ESPComponents[player] = {box=box,healthBar=healthBar,healthBarBackground=healthBarBackground,healthBarBorder=healthBarBorder,healthText=healthText,nameText=nameText,distanceText=distanceText,weaponText=weaponText,tracer=tracer,arrow=arrow,skeletonLines=skeletonLines,skeletonPoints=skeletonPoints};
	RunService.RenderStepped:Connect(function()
		...
		if (rootPart and humanoid and (humanoid.Health > 0)) then
			...
			if (getgenv().OutOfViewArrows and not onScreen) then
				...
				local screenPosition = Vector2.new((Camera.ViewportSize.X / 2) + ((dotProduct * Camera.ViewportSize.X) / 3), (Camera.ViewportSize.Y / 2) - ((crossProduct.Y * Camera.ViewportSize.Y) / 3));
				screenPosition = Vector2.new(math.clamp(screenPosition.X, getgenv().ArrowSize, Camera.ViewportSize.X - getgenv().ArrowSize), math.clamp(screenPosition.Y, getgenv().ArrowSize, Camera.ViewportSize.Y - getgenv().ArrowSize));
				...
				arrow.PointB = Vector2.new(screenPosition.X - (getgenv().ArrowSize * math.cos(angle - 0.5)), screenPosition.Y - (getgenv().ArrowSize * math.sin(angle - 0.5)));
				arrow.PointC = Vector2.new(screenPosition.X - (getgenv().ArrowSize * math.cos(angle + 0.5)), screenPosition.Y - (getgenv().ArrowSize * math.sin(angle + 0.5)));
				arrow.Color = getgenv().ArrowColor;
				arrow.Visible = true;
			else
				arrow.Visible = false;
...
local function updateRadar()
	if not getgenv().ShowRadar then
		...
		for _, player in pairs(radarPlayers) do
			if player.dot then
				player.dot.Visible = false;
			end
			if player.direction then
				player.direction.Visible = false;
			end
			if player.name then
	...
	for _, player in pairs(Players:GetPlayers()) do
		if (player.Character and player.Character:FindFirstChild("HumanoidRootPart") and (player ~= LocalPlayer)) then
			...
			if not radarPlayers[player] then
				radarPlayers[player] = {dot=Drawing.new("Circle"),direction=Drawing.new("Line"),name=Drawing.new("Text")};
...
Toggle = TabHandles.W:Toggle({Title="透视开启",Value=false,Callback=function(Value)
	getgenv().ESPEnabled = Value;
end});
Toggle = TabHandles.W:Toggle({Title="模型透视",Value=false,Callback=function(Value)
	getgenv().ShowSkeleton = Value;
end});
Toggle = TabHandles.W:Toggle({Title="方框透视",Value=false,Callback=function(Value)
	getgenv().ShowBox = Value;
end});
Toggle = TabHandles.W:Toggle({Title="射线透视",Value=false,Callback=function(Value)
	getgenv().ShowTracer = Value;
end});
local bulletTrackingEnabled = true;
local oldHook = nil;
Toggle = TabHandles.W:Toggle({Title="名字透视",Value=false,Callback=function(Value)
