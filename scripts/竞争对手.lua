...
PlayStartSound:Play();
PlayStartSound.Ended:Connect(function()
	PlayStartSound:Destroy();
end);
...
do
	...
	elseif RunService:IsStudio() then
		...
		WindUI = loadstring(game:HttpGet("https://raw.githubusercontent.com/yisanll/yisani/refs/heads/main/ui.lua.txt"))();
	end
end
...
local AimSettings = {Enabled=false,FOV=100,Smoothness=10,CrosshairDistance=5,FOVColor=Color3.fromRGB(0, 255, 0),FriendCheck=true,WallCheck=true,TargetPlayer=nil,TargetAll=true,FOVRainbowEnabled=true,FOVRainbowSpeed=8,FOVEnabled=true};
			...
			if (hrp and humanoid and (humanoid.Health > 0)) then
				...
				if (distance < 30) then
					...
					if (dotProduct < 0.5) then
						WindUI:Notify({Title="偷袭警告",Content=("小心有人要偷袭你：" .. player.Name),Icon="alert-triangle",Color=Color3.fromRGB(255, 100, 100),Duration=5});
...
local BLTab = Window:Tab({Title="暴力杀戮",Icon="crown",IconColor=Blue,IconShape="Square",Border=true});
BLTab:Toggle({Title="Ragebot",Value=false,Callback=function(state)
	if state then
				...
				if (itm and trg) then
					...
					if (hd and ch and ch:FindFirstChild("HumanoidRootPart")) then
						...
						if wallCheck(cp, tp, hd) then
							...
							evt:FireServer(itm:Get("ObjectID"), "\x1A", {["\x01"]={["\x00"]=encf(cp, targetPosition),["\x01"]=encf(targetPosition, cp),["\x02"]=hd,["\x03"]={["\x00"]=dir.X,["\x01"]=dir.Y,["\x02"]=dir.Z,["\x03"]=-dir.X,["\x04"]=-dir.Y,["\x05"]=-dir.Z}},["\x02"]=true,["\x03"]=true}, nil);
...
local PlayerTab = Window:Tab({Title="本地玩家",Icon="user",IconColor=Blue,IconShape="Square",Border=true});
do
	...
	PlayerTab:Input({Title="设置快速跑步速度",Desc="输入速度 (默认: 1)",Placeholder="输入速度",Callback=function(value)
		...
		if numValue then
			...
			WindUI:Notify({Title="依旧错误来了",Content="请输入有效数字",Icon="alert-circle",Color=Red});
		end
	end});
...
local AimTab = Window:Tab({Title="暴力功能",Icon="crosshair",IconColor=Red,IconShape="Square",Border=true});
...
do
	...
	QuickSettings:Button({Title="快速设置: 近距离(强锁)",Desc="FOV: 80 平滑: 1 预判0",Justify="Center",Callback=function()
		AimSettings.FOV = 80;
	...
	QuickSettings:Space();
	QuickSettings:Button({Title="快速设置: 中距离(小强锁)",Desc="FOV: 120, 平滑: 4 预判2",Justify="Center",Callback=function()
		AimSettings.FOV = 120;
	...
	QuickSettings:Space();
	QuickSettings:Button({Title="快速设置: 远距离",Desc="FOV: 130 平滑: 5 预判3",Justify="Center",Callback=function()
		AimSettings.FOV = 130;
...
local OtherTab = Window:Tab({Title="次要功能",Icon="settings",IconColor=Green,IconShape="Square",Border=true});
do
...
local MusicTab = Window:Tab({Title="音乐库",Icon="music",IconColor=Grey,IconShape="Square",Border=true});
local activeSounds = {};
local customMusic = nil;
local customMusicPlaying = false;
do
	MusicTab:Section({Title="音乐库",TextSize=20,FontWeight=Enum.FontWeight.Bold});
	MusicTab:Space();
	MusicTab:Section({Title="预定义音乐列表",TextSize=16,TextTransparency=0.3});
	MusicTab:Space();
	MusicTab:Button({Title="关闭所有音乐",Color=Color3.fromHex("#FF3B30"),Justify="Center",Callback=function()
		...
		WindUI:Notify({Icon="stop",Title="音乐已停止",Content="所有正在播放的音乐已关闭"});
	end,Icon="stop",IconAlign="Left"});
	MusicTab:Space();
	local musicList = {{"柔慢日语歌","rbxassetid://88942576563851"},{"唯一","rbxassetid://138570939058838"},{"Qian Li","rbxassetid://9042630735"},{"DJ","rbxassetid://112834898401032"}};
	for _, music in ipairs(musicList) do
		local title, soundId = music[1], music[2];
		MusicTab:Button({Title=title,Color=Color3.fromHex("#257AF7"),Justify="Center",Callback=function()
			local sound = Instance.new("Sound");
			sound.SoundId = soundId;
			sound.Parent = workspace;
			sound:Play();
			table.insert(activeSounds, sound);
			WindUI:Notify({Icon="play",Title="播放音乐",Content=("正在播放: " .. title)});
		...
		end,Icon="music",IconAlign="Left"});
	end
end
local AboutTab = Window:Tab({Title="关于作者",Icon="info",IconColor=Grey,IconShape="Square",Border=true});
do
	...
	AboutTab:Space();
	AboutTab:Button({Title="重置所有配置",Color=Yellow,Justify="Center",Callback=function()
		AimSettings = {Enabled=false,FOV=100,Smoothness=10,CrosshairDistance=5,FOVColor=Color3.fromRGB(0, 255, 0),FriendCheck=true,WallCheck=true,TargetPlayer=nil,TargetAll=true,FOVRainbowEnabled=true,FOVRainbowSpeed=8,FOVEnabled=true};
		...
		WindUI:Notify({Title="重置",Content="所有设置已重置",Icon="refresh-cw"});
	end});
end
...
WindUI:Notify({Title="pvp",Content="TnineHub!",Icon="check-circle",Duration=5});
