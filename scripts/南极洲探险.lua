...
local function ApplyFallBypass()
	if getgenv().FallDamageHookLoaded then
		return;
	end
	pcall(function()
...
MainTab:Toggle({Title="开启无掉落伤害",Value=false,Callback=function(Value)
...
MainTab:Slider({Title="加速",Step=1,Value={Min=16,Max=200,Default=16},Callback=function(v)
	tpSpeedValue = tonumber(v) or 16;
end});
MainTab:Toggle({Title="开启加速",Callback=function(state)
	tpWalkEnabled = state;
	if state then
		task.spawn(function()
...
local jump_Ys = nil;
MainTab:Toggle({Title="开启无限跳",Callback=function(v)
	getgenv().Jump = v;
	if jump_Ys then
		jump_Ys:Disconnect();
		jump_Ys = nil;
