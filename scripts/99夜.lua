...
local Tabs = {Main=Window:Tab({Title="主要功能",Icon="zap"}),Ninja=Window:Tab({Title="传送",Icon="user"}),ESP=Window:Tab({Title="透视",Icon="dumbbell"})};
Window:SelectTab(1);
...
local Features = {KillAura=false,AutoChop=false,AutoEat=false,ChildESP=false,ChestESP=false,InstantInteract=false};
local Blacklist = {};
local ClientModule;
local EatRemote;
...
local function createESP(target, name)
	...
	if (not rootPart or espData[target]) then
		return;
	end
...
local itemConfig = {{name="Log",display="木头",espColor=Color3.fromRGB(139, 69, 19)},{name="Carrot",display="胡萝卜",espColor=Color3.fromRGB(255, 165, 0)},{name="Berry",display="浆果",espColor=Color3.fromRGB(255, 0, 0)},{name="Bolt",display="螺栓",espColor=Color3.fromRGB(255, 255, 0)},{name="Broken Fan",display="风扇",espColor=Color3.fromRGB(100, 100, 100)},{name="Coal",display="煤炭",espColor=Color3.fromRGB(0, 0, 0)},{name="Coin Stack",display="钱堆",espColor=Color3.fromRGB(255, 215, 0)},{name="Fuel Canister",display="燃料罐",espColor=Color3.fromRGB(255, 50, 50)},{name="Item Chest",display="宝箱",espColor=Color3.fromRGB(210, 180, 140)},{name="Old Flashlight",display="手电筒",espColor=Color3.fromRGB(200, 200, 200)},{name="Old Radio",display="收音机",espColor=Color3.fromRGB(150, 150, 150)},{name="Rifle Ammo",display="步枪子弹",espColor=Color3.fromRGB(150, 75, 0)},{name="Revolver Ammo",display="左轮子弹",espColor=Color3.fromRGB(150, 75, 0)},{name="Sheet Metal",display="金属板",espColor=Color3.fromRGB(192, 192, 192)},{name="Revolver",display="左轮",espColor=Color3.fromRGB(75, 75, 75)},{name="Rifle",display="步枪",espColor=Color3.fromRGB(75, 75, 75)},{name="Bandage",display="绷带",espColor=Color3.fromRGB(255, 240, 245)},{name="Crossbow Cultist",display="敌人",espColor=Color3.fromRGB(255, 0, 0)},{name="Bear",display="熊",espColor=Color3.fromRGB(139, 69, 19)},{name="Alpha Wolf",display="阿尔法狼",espColor=Color3.fromRGB(128, 128, 128)},{name="Wolf",display="狼",espColor=Color3.fromRGB(192, 192, 192)},{name="Chair",display="椅子",espColor=Color3.fromRGB(160, 82, 45)},{name="Tyre",display="轮胎",espColor=Color3.fromRGB(20, 20, 20)},{name="Alien Chest",display="外星宝箱",espColor=Color3.fromRGB(0, 255, 0)},{name="Chest",display="宝箱",espColor=Color3.fromRGB(210, 180, 140)},{name="Lost Child",display="走失的孩子",espColor=Color3.fromRGB(0, 255, 255)},{name="Lost Child1",display="走失的孩子1",espColor=Color3.fromRGB(0, 255, 255)},{name="Lost Child2",display="走失的孩子2",espColor=Color3.fromRGB(0, 255, 255)},{name="Lost Child3",display="走失的孩子3",espColor=Color3.fromRGB(0, 255, 255)},{name="Dino Kid",display="恐龙孩子",espColor=Color3.fromRGB(0, 255, 255)},{name="kraken kid",display="海怪孩子",espColor=Color3.fromRGB(0, 255, 255)},{name="Squid kid",display="鱿鱼孩子",espColor=Color3.fromRGB(0, 255, 255)},{name="Koala Kid",display="考拉孩子",espColor=Color3.fromRGB(0, 255, 255)},{name="koala",display="考拉",espColor=Color3.fromRGB(0, 255, 255)}};
...
local function teleportItemsToPlayer(itemName, displayName)
	...
	WindUI:Notify({Title="成功",Content=("已将" .. #items .. "个" .. displayName .. "传送到你旁边"),Duration=2});
end
local function toggleESP(itemName, displayName, color)
	...
	WindUI:Notify({Title="提示",Content=("已开启" .. displayName .. "透视 (" .. #items .. "个)"),Duration=2});
end
...
RunService.Heartbeat:Connect(function()
	...
	elseif instantInteractConn then
...
Tabs.Main:Toggle({Title="杀戮光环",Description="自动攻击附近敌人",Value=false,Callback=function(value)
	Features.KillAura = value;
end});
Tabs.Main:Toggle({Title="自动砍树",Description="自动砍伐附近树木",Value=false,Callback=function(value)
	Features.AutoChop = value;
end});
Tabs.Main:Toggle({Title="自动进食",Description="自动吃附近食物",Value=false,Callback=function(value)
	Features.AutoEat = value;
end});
Tabs.Main:Toggle({Title="瞬间互动",Description="立即完成所有互动",Value=false,Callback=function(value)
	Features.InstantInteract = value;
end});
Tabs.Ninja:Button({Title="传送回篝火",Callback=teleportToBonfire});
for _, item in ipairs(itemConfig) do
	Tabs.Ninja:Button({Title=("传送到" .. item.display),Callback=function()
		teleportToItem(item.name, item.display);
	end});
	Tabs.Ninja:Button({Title=("召唤" .. item.display),Callback=function()
		teleportItemsToPlayer(item.name, item.display);
	end});
end
Tabs.ESP:Toggle({Title="99夜透视 (宝箱&孩子)",Value=false,Callback=toggle99NightESP});
for _, item in ipairs(itemConfig) do
	Tabs.ESP:Button({Title=(item.display .. "透视"),Callback=function()
		toggleESP(item.name, item.display, item.espColor);
	end});
end
Tabs.ESP:Button({Title="清除所有透视",Callback=function()
	for _, item in ipairs(itemConfig) do
