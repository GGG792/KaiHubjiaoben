local AimSettings = {Enabled=false,FOV=100,FOVEnabled=true,FOVRainbowEnabled=false,FOVRainbowSpeed=5,FOVColor=Color3.fromRGB(255, 255, 255),Smoothness=5,CrosshairDistance=0,WallCheck=false,FriendCheck=false,TargetAll=true,TargetPlayer=nil};
...
local function UpdateESPColors()
	if (not ESPEnabled or not ESPFolder) then
		return;
	end
	pcall(function()
...
local function CheckBackstabThreat()
	if not BackstabCheckEnabled then
		return;
	end
				...
				if (hrp and humanoid and (humanoid.Health > 0)) then
					...
					if (distance < 30) then
						...
						if (dotProduct < 0.5) then
							WindUI:Notify({Title="有人偷袭",Content=("小心有人要偷袭你：" .. player.Name),Icon="alert-triangle",Color=Color3.fromRGB(255, 100, 100),Duration=5});
		...
		pcall(function()
			...
			humanoid.Died:Connect(function()
				if DeathCheckEnabled then
					WindUI:Notify({Title="死亡提醒",Content="受着",Icon="skull",Color=Color3.fromRGB(255, 0, 0),Duration=8});
				end
			end);
		end);
	end);
...
local PlayerTab = Window:Tab({Title="本地玩家",Icon="crown",Locked=false});
Window:SelectTab(1);
do
	PlayerTab:Section({Title="主要功能",TextSize=16,FontWeight=Enum.FontWeight.SemiBold});
	PlayerTab:Toggle({Title="无限跳跃",Desc="启用后可以无限跳跃",Callback=function(enabled)
	...
	PlayerTab:Space();
	PlayerTab:Input({Title="设置重力",Desc=("输入重力值 (默认:196" .. tostring(originalGravity) .. ")"),Placeholder="输入重力值",Callback=function(value)
		...
		if numValue then
			...
			WindUI:Notify({Title="错误来了",Content="请输入数字",Icon="alert-circle",Color=Red});
		end
	end});
	PlayerTab:Space();
	PlayerTab:Input({Title="设置快速跑步速度",Desc="输入速度 (默认: 1)",Placeholder="输入速度",Callback=function(value)
		local numValue = tonumber(value);
		if numValue then
			SpeedValue = numValue;
			WindUI:Notify({Title="速度设置",Content=("速度已设置为: " .. tostring(numValue) .. "速度"),Icon="zap"});
		else
			WindUI:Notify({Title="错误来了",Content="请输入有效数字",Icon="alert-circle",Color=Red});
		end
	end});
	PlayerTab:Toggle({Title="开启快速跑步",Desc="启用快速跑步功能",Callback=function(enabled)
...
local AimTab = Window:Tab({Title="自瞄设置",Icon="crown",Locked=false});
...
local function CreateRainbowUI()
	...
	local rainbowColors = {Color3.fromRGB(255, 0, 0),Color3.fromRGB(255, 95, 0),Color3.fromRGB(255, 165, 0),Color3.fromRGB(255, 215, 0),Color3.fromRGB(255, 255, 0),Color3.fromRGB(144, 238, 144),Color3.fromRGB(0, 255, 0),Color3.fromRGB(0, 200, 200),Color3.fromRGB(0, 0, 255),Color3.fromRGB(75, 0, 130),Color3.fromRGB(138, 43, 226),Color3.fromRGB(148, 0, 211),Color3.fromRGB(199, 21, 133),Color3.fromRGB(255, 20, 147)};
	local rainbowColors2 = {Color3.fromRGB(255, 0, 0),Color3.fromRGB(255, 127, 0),Color3.fromRGB(255, 255, 0),Color3.fromRGB(0, 255, 0),Color3.fromRGB(0, 0, 255),Color3.fromRGB(75, 0, 130),Color3.fromRGB(148, 0, 211)};
	local timeOffset = 0;
	...
	local function handleClick()
		...
		local clickScaleUp = TweenService:Create(rainbowBackground, tweenInfo1, {Size=(originalSize * 0.7),Position=UDim2.new(0.15, 0, 0.15, 0)});
		local clickScaleDown = TweenService:Create(rainbowBackground, tweenInfo2, {Size=originalSize,Position=originalPosition});
		local originalStrokeColor = rainbowStroke.Color;
		local flashTween = TweenService:Create(rainbowStroke, tweenInfo1, {Color=Color3.fromRGB(255, 255, 255)});
		local revertStroke = TweenService:Create(rainbowStroke, tweenInfo2, {Color=originalStrokeColor});
		clickScaleUp:Play();
		flashTween:Play();
	...
	mainFrame.MouseEnter:Connect(function()
		...
		local tween1 = TweenService:Create(rainbowStroke, tweenInfo, {Thickness=6});
		pulseAmount = 0.15;
		tween1:Play();
	end);
	mainFrame.MouseLeave:Connect(function()
		local tweenInfo = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out);
		local tween1 = TweenService:Create(rainbowStroke, tweenInfo, {Thickness=3});
		pulseAmount = 0.1;
		tween1:Play();
	end);
	rainbowBackground.BackgroundTransparency = 1;
	rainbowStroke.Transparency = 1;
	local fadeIn = TweenService:Create(rainbowBackground, TweenInfo.new(0.5), {BackgroundTransparency=0});
	local strokeFadeIn = TweenService:Create(rainbowStroke, TweenInfo.new(0.5), {Transparency=0});
	task.wait(0.2);
	fadeIn:Play();
	strokeFadeIn:Play();
	return true;
...
local function ToggleRainbowUI(state)
	...
	if state then
		...
		if success then
			WindUI:Notify({Title="自瞄快捷UI",Content="快捷UI 让你秒人更加高效",Icon="sparkles"});
		end
	else
		if RainbowUIScreenGui then
			RainbowUIScreenGui:Destroy();
		...
		WindUI:Notify({Title="自瞄快捷UI",Content="快捷UI已隐藏",Icon="sparkles"});
	end
end
do
	AimTab:Section({Title="自瞄设置",TextSize=16,FontWeight=Enum.FontWeight.SemiBold});
	AimTab:Toggle({Title="启用自瞄",Desc="开启/关闭自瞄功能",Callback=function(enabled)
	...
	AimTab:Space();
	AimTab:Toggle({Title="自瞄快捷UI",Desc="快捷UI 让你秒人更加高效",Callback=function(enabled)
		ToggleRainbowUI(enabled);
	end});
	AimTab:Toggle({Title="FOV开关",Desc="显示自瞄范围圆圈",Value=AimSettings.FOVEnabled,Callback=function(enabled)
		AimSettings.FOVEnabled = enabled;
		UpdateFOVCircle();
	end});
	AimTab:Toggle({Title="FOV彩虹效果",Desc="开启FOV圆圈彩虹效果",Value=AimSettings.FOVRainbowEnabled,Callback=function(enabled)
		AimSettings.FOVRainbowEnabled = enabled;
		UpdateFOVCircle();
	end});
	AimTab:Slider({Title="FOV彩虹速度",Desc="调整彩虹流动的速度",Value={Min=1,Max=20,Default=AimSettings.FOVRainbowSpeed},Callback=function(value)
		AimSettings.FOVRainbowSpeed = value;
	end});
	AimTab:Space();
	AimTab:Slider({Title="自瞄范围 (FOV)",Desc="设置自瞄FOV大小",Value={Min=50,Max=500,Default=AimSettings.FOV},Callback=function(value)
		AimSettings.FOV = value;
		UpdateFOVCircle();
	end});
	AimTab:Space();
	AimTab:Slider({Title="自瞄平滑度",Desc="数值越小越强锁",Value={Min=1,Max=50,Default=AimSettings.Smoothness},Callback=function(value)
		AimSettings.Smoothness = value;
	end});
	AimTab:Space();
	AimTab:Slider({Title="预判距离",Desc="设置预判距离(需要强锁直接调到0-3)",Value={Min=0,Max=20,Default=AimSettings.CrosshairDistance},Callback=function(value)
		AimSettings.CrosshairDistance = value;
	end});
	AimTab:Space();
	AimTab:Colorpicker({Title="FOV圆圈颜色",Desc="彩虹模式关闭时生效",Default=AimSettings.FOVColor,Callback=function(color)
		AimSettings.FOVColor = color;
		UpdateFOVCircle();
	end});
	AimTab:Space();
	AimTab:Toggle({Title="好友检测",Desc="不秒好友",Value=AimSettings.FriendCheck,Callback=function(enabled)
		AimSettings.FriendCheck = enabled;
	end});
	AimTab:Space();
	AimTab:Toggle({Title="墙壁检测",Desc="开启墙壁检测 避免自瞄乱飞",Value=AimSettings.WallCheck,Callback=function(enabled)
		AimSettings.WallCheck = enabled;
	end});
	AimTab:Space();
	AimTab:Toggle({Title="队伍检测",Desc="不攻击同队队友",Value=AimTeamCheck,Callback=function(enabled)
		AimTeamCheck = enabled;
	end});
	AimTab:Space();
	AimTab:Toggle({Title="目标自瞄模式",Desc="开启后可以选择目标进行制裁",Value=false,Callback=function(enabled)
	...
	local targetDropdown = AimTab:Dropdown({Title="选择目标玩家",Desc="选择要自瞄的玩家",Values=playerList,Value=nil,AllowNone=true,Callback=function(selected)
	...
	AimTab:Space();
	AimTab:Section({Title="自瞄部位设置",TextSize=16,FontWeight=Enum.FontWeight.SemiBold});
	AimTab:Dropdown({Title="自瞄部位",Desc="选择要瞄准的身体部位",Values={"头","上身","左腿","右腿","鸡巴","奶子"},Value=AimTargetPart,Callback=function(selected)
		AimTargetPart = selected;
	end});
	AimTab:Space();
	AimTab:Section({Title="黑名单管理",TextSize=16,FontWeight=Enum.FontWeight.SemiBold});
	blacklistInput = AimTab:Input({Title="自瞄黑名单",Desc="输入不攻击的玩家名字，多个用逗号分隔",Placeholder="例如: Player1,Player2,Player3",Callback=function(value)
		local names = {};
	...
	AimTab:Button({Title="添加当前目标到黑名单",Justify="Center",Callback=function()
		if (CurrentTarget and CurrentTarget.Name) then
			local targetName = CurrentTarget.Name;
			for _, name in ipairs(AimBlacklist) do
				if (name == targetName) then
					WindUI:Notify({Title="黑名单",Content=(targetName .. " 已在黑名单中"),Icon="info"});
					return;
				end
			end
			...
			if (blacklistInput and blacklistInput.SetValue) then
				...
				WindUI:Notify({Title="黑名单",Content=("已添加 " .. targetName .. "，请手动更新输入框"),Icon="info"});
			end
		else
			WindUI:Notify({Title="黑名单",Content="没有当前目标",Icon="alert-circle"});
		end
	end});
	AimTab:Space();
	AimTab:Button({Title="清空白名单",Justify="Center",Callback=function()
		AimBlacklist = {};
		if (blacklistInput and blacklistInput.SetValue) then
			blacklistInput:SetValue("");
		end
		WindUI:Notify({Title="黑名单",Content="黑名单已清空",Icon="check"});
	end});
	AimTab:Space();
	...
	AimTab:Section({Title=statusText,TextSize=14,FontWeight=Enum.FontWeight.Medium,TextColor=((AimSettings.Enabled and Green) or Grey)});
	local QuickSettings = AimTab:Group({});
	QuickSettings:Button({Title="快速设置: 强锁[子弹有延迟类]",Desc="FOV99 平滑1 预判0.96",Justify="Center",Callback=function()
		AimSettings.FOV = 99;
	...
	QuickSettings:Space();
	QuickSettings:Button({Title="快速设置: 强锁[子弹无延迟]",Desc="FOV120, 平滑1 预判0",Justify="Center",Callback=function()
		AimSettings.FOV = 120;
		AimSettings.Smoothness = 1;
		AimSettings.CrosshairDistance = 0;
		UpdateFOVCircle();
		WindUI:Notify({Title="快速设置",Content="已使用强锁设置",Icon="settings"});
	end});
	QuickSettings:Space();
	QuickSettings:Button({Title="快速设置: 平滑类[]",Desc="FOV130 平滑6 预判1",Justify="Center",Callback=function()
		AimSettings.FOV = 130;
...
local OtherTab = Window:Tab({Title="绘制功能",Icon="crown",Locked=false});
do
	OtherTab:Section({Title="ESP",TextSize=16,FontWeight=Enum.FontWeight.SemiBold});
	OtherTab:Toggle({Title="玩家透视 (ESP)",Desc="显示玩家描边和距离",Callback=function(enabled)
		ToggleESP(enabled);
	end});
	OtherTab:Space();
	OtherTab:Colorpicker({Title="ESP玩家名字颜色",Desc="设置玩家名字显示颜色",Default=ESPNameColor,Callback=function(color)
		ESPNameColor = color;
		if (ESPEnabled and not ESPRainbowEnabled) then
			UpdateESPColors();
		end
	end});
	OtherTab:Colorpicker({Title="ESP身体绘制颜色",Desc="设置玩家身体颜色",Default=ESPBodyColor,Callback=function(color)
		ESPBodyColor = color;
		if (ESPEnabled and not ESPRainbowEnabled) then
			UpdateESPColors();
		end
	end});
	OtherTab:Slider({Title="ESP玩家名字大小",Desc="设置玩家名字的文本大小",Value={Min=8,Max=24,Default=ESPNameSize},Callback=function(value)
		ESPNameSize = value;
		if ESPEnabled then
			UpdateESPNameSize();
		end
	...
	OtherTab:Space();
	OtherTab:Toggle({Title="ESP彩虹渐变",Desc="开启透视彩虹效果",Callback=function(enabled)
		ESPRainbowEnabled = enabled;
		if ESPEnabled then
			UpdateESPColors();
		end
	end});
	OtherTab:Slider({Title="ESP彩虹速度",Desc="调整彩虹的速度",Value={Min=1,Max=10,Default=ESPRainbowSpeed},Callback=function(value)
		ESPRainbowSpeed = value;
	end});
	OtherTab:Space();
	OtherTab:Slider({Title="ESP最大显示距离",Desc="设置ESP显示的最大距离（单位：studs）",Value={Min=50,Max=10000,Default=ESPMaxDistance},Callback=function(value)
		ESPMaxDistance = value;
	end});
	OtherTab:Space();
	OtherTab:Toggle({Title="队伍检测",Desc="开启后只显示敌方队伍",Value=ESPTeamCheck,Callback=function(enabled)
	...
	OtherTab:Space();
	OtherTab:Toggle({Title="偷袭检测提醒",Desc="检测背后或侧面的敌人并提醒",Callback=function(enabled)
		BackstabCheckEnabled = enabled;
		WindUI:Notify({Title="偷袭检测",Content=((enabled and "偷袭检测已开启") or "偷袭检测已关闭"),Icon="shield-alert"});
	end});
	OtherTab:Toggle({Title="死亡提醒",Desc="玩家死亡时显示提醒消息",Callback=function(enabled)
		DeathCheckEnabled = enabled;
		if enabled then
			SetupDeathDetection();
		end
		WindUI:Notify({Title="死亡提醒",Content=((enabled and "死亡提醒已开启") or "死亡提醒已关闭"),Icon="heart"});
	end});
	OtherTab:Space();
	OtherTab:Toggle({Title="夜视模式",Desc="开启夜间模式",Callback=function(enabled)
		...
		if enabled then
			...
			WindUI:Notify({Title="夜视模式",Content="夜视模式已开启",Icon="moon"});
		else
			Lighting.Brightness = originalBrightness;
			Lighting.Ambient = originalAmbient;
			Lighting.OutdoorAmbient = Color3.fromRGB(0.5, 0.5, 0.5);
			WindUI:Notify({Title="夜视模式",Content="夜视模式已关闭",Icon="moon"});
		end
	end});
end
local KillTab = Window:Tab({Title="杀戮与主要",Icon="crown",Locked=false});
local originalFire = nil;
KillTab:Toggle({Title="子追",Value=false,Callback=function(state)
	...
	elseif originalFire then
		bullet_handler.Fire = originalFire;
	end
end});
local autoLobbyEnabled = false;
local autoLobbyThread = nil;
KillTab:Toggle({Title="自动加入对战",Value=false,Callback=function(state)
...
do
	...
	local function getVisiblePart(targetCharacter)
		if (not targetCharacter or not LocalPlayer.Character) then
			return nil;
		end
	...
	local function createBeam(startPos, endPos)
		...
		task.delay(math.random(10, 40) / 10, function()
			shaking = false;
			for i = 0, 1, 0.05 do
				if (not part1 or not part1.Parent) then
					break;
				end
	...
	local function shoot(player, targetPart, targetPos, origin)
		...
		if ((currentTime - lastShotTime) < 0.56) then
			return false;
		end
		local character = LocalPlayer.Character;
		if (not character or not targetPart or not origin) then
			return false;
		end
		...
		if clientRemotes then
			pcall(function()
				clientRemotes.CheckFire:FireServer(time, origin);
			end);
			pcall(function()
				clientRemotes.CheckShot:FireServer(0, 0, 1, 0.8, cframe, targetPos, targetPart, 11, time);
			end);
			pcall(function()
				clientRemotes.Reload:FireServer();
			end);
		end
		...
		if gun then
			...
			if gun then
				...
				if gun then
					pcall(function()
						gun.ProjectileRender:FireServer(time, character, origin, direction * 999999, 360, 0, Vector3.zero, 5, "Bullet");
					end);
					pcall(function()
						gun.ProjectileFinished:FireServer(time, CFrame.new(targetPos), "Gib_T", false, 15, "rbxassetid://2814354338");
					end);
				end
			end
		end
