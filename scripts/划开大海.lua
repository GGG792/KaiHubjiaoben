...
Window:SelectTab(1);
CombatTab:Toggle({Title="自动出售",Value=false,Callback=function(state)
...
CombatTab:Toggle({Title="自动获取鱼",Value=false,Callback=function(state)
...
CombatTab:Slider({Title="移动速度",Step=1,Value={Min=16,Max=200,Default=16},Callback=function(v)
	tpSpeedValue = tonumber(v) or 16;
end});
CombatTab:Toggle({Title="开启加速",Value=false,Callback=function(state)
	tpWalkEnabled = state;
	if state then
		task.spawn(function()
...
Window:Tag({Title="杨志卡",Radius=5,Color=Color3.fromHex("#FFB347")});
