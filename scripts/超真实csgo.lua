...
local Tabcsgo = Window:Tab({Title="暴力杀戮",Icon="crown"});
Window:SelectTab(1);
Tabcsgo:Toggle({Title="杀死所有人",Icon="check",Value=false,Callback=function(state)
	local shouldKill = state;
	getgenv().KillAllEnabled = shouldKill;
	if shouldKill then
		if not getgenv().KillAllTask then
			getgenv().KillAllTask = task.spawn(function()
				while true do
					local isEnabled = getgenv().KillAllEnabled;
					if isEnabled then
						local Players = game:GetService("Players");
						local LocalPlayer = Players.LocalPlayer;
						local ReplicatedStorage = game:GetService("ReplicatedStorage");
						local MeleeAttackEvent = ReplicatedStorage:FindFirstChild("Events");
						if MeleeAttackEvent then
							MeleeAttackEvent = MeleeAttackEvent:FindFirstChild("MeleeAttack");
						end
						if (not MeleeAttackEvent or not MeleeAttackEvent:IsA("RemoteEvent")) then
							warn("错误");
						...
						for _, player in pairs(Players:GetPlayers()) do
							if ((player ~= LocalPlayer) and player.Character) then
								...
								if (humanoid and (humanoid.Health > 0) and head) then
									local attackData = {{rayInstance=head,rayPosition=head.Position,rayMaterial=Enum.Material.Plastic,rayNormal=Vector3.new(0, 1, 0)},head.Position,Vector3.new(0, 1, 0),8,{MainDamage=math.huge,BackstabDamage=math.huge,AltDamage=math.huge},2,false,((getgenv().text and getgenv().text[math.random(#getgenv().text or {"默认文本"})]) or "攻击")};
									MeleeAttackEvent:FireServer(unpack(attackData));
								end
							end
						end
					else
