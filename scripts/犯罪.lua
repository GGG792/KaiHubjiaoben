...
local functions = {Fullbright=false,AutoOpenDoors=false,NoBarriers=false,NoGrinder=false,FastPickup=false,AutoPickupScraps=false,AutoPickupTools=false,AutopickupCrates=false,AutoPickupMoney=false,Infstamina=false,Nofalldamage=false,Noclip=false,FakeDown=false,Stopneckmove=false,Unbreaklimbs=false,SilentAim=false,Instantreload=false,Meleeaura=false,RageBot=false,TrigerBot=false,RocketControl=false,ESP=false,ArmsChams=false,ToolsChams=false};
local SectionSettings = {SilentAim={Draw=false,DrawSize=50,DrawColor=Color3.new(1, 1, 1),TargetParts={"Head"},CheckDowned=false,CheckWall=false,CheckTeam=false,CheckWhiteList=false},Aimbot={Draw=false,DrawSize=50,DrawColor=Color3.new(1, 1, 1),TargetParts={"Head"},CheckDowned=false,CheckWall=false,CheckTeam=false,CheckWhiteList=false,Velocity=false,Smooth=false,SmoothSize=0.5},MeleeAura={ShowAnim=false,TargetParts={"Head"},CheckDowned=false,CheckTeam=false,CheckWhiteList=false,Distance=15},RageBot={CheckDowned=false,CheckWhiteList=false},ESP={Name=false,Box=false,Weapon=false,Highlight=false}};
local Methods = {Fly="Bypass",Infstamina="Getgc"};
local cockie = {SilentAimCircle=nil,SilentAim_body=nil,ESPHighlight=nil,AimBotCircle=nil,aimbot_button=nil,Aimbot_body=nil,MeleeAura_body=nil};
local RUNS = {cameraFOV=nil,JumpHeight=nil,AutoOpenDoors=nil,AutopickupScraps=nil,AutopickupTools=nil,AutopickupCrates=nil,AutopickupMoney=nil,Infstamina=nil,Fly=nil,Noclip=nil,Meleeaura=nil,ESP=nil};
local funcindex = {Fullbright={oldClockTime=nil,oldBrightness=nil}};
local WhiteList = {};
function CharStats(plr)
	local folder = rp.CharStats[plr.Name];
	return folder;
end
Tabs.World:Toggle({Title="夜视",Desc="地图变亮",Value=functions.Fullbright,Callback=function(Value)
...
Tabs.World:Toggle({Title="自动开门",Desc="自动打开附近的门",Value=functions.AutoOpenDoors,Callback=function(Value)
	...
	elseif RUNS.AutoOpenDoors then
		RUNS.AutoOpenDoors:Disconnect();
		RUNS.AutoOpenDoors = nil;
	end
end});
Tabs.World:Toggle({Title="无屏障",Desc="移除地图屏障",Value=functions.NoBarriers,Callback=function(Value)
...
Tabs.World:Toggle({Title="防研磨机",Desc="防止研磨机伤害",Value=functions.NoGrinder,Callback=function(Value)
...
Tabs.World:Toggle({Title="快速拾取",Desc="瞬间拾取物品",Value=functions.FastPickup,Callback=function(Value)
...
Tabs.World:Toggle({Title="自动拾取废料",Desc="自动拾取附近的废料",Value=functions.AutoPickupScraps,Callback=function(Value)
	...
	elseif RUNS.AutopickupScraps then
		RUNS.AutopickupScraps:Disconnect();
		RUNS.AutopickupScraps = nil;
	end
end});
Tabs.World:Toggle({Title="自动拾取工具",Desc="自动拾取附近的工具",Value=functions.AutoPickupTools,Callback=function(Value)
	...
	elseif RUNS.AutopickupTools then
		RUNS.AutopickupTools:Disconnect();
		RUNS.AutopickupTools = nil;
	end
end});
Tabs.World:Toggle({Title="自动拾取金钱",Desc="自动拾取附近的金钱",Value=functions.AutoPickupMoney,Callback=function(Value)
	...
	elseif RUNS.AutopickupMoney then
		RUNS.AutopickupMoney:Disconnect();
		RUNS.AutopickupMoney = nil;
	end
end});
Tabs.Player:Slider({Title="FOV",Desc="调整相机视野",Value={Min=70,Max=120,Default=camera.FieldOfView},Callback=function(Value)
	if RUNS.cameraFOV then
...
Tabs.Player:Slider({Title="相机距离",Desc="调整相机最大距离",Value={Min=10,Max=500,Default=me.CameraMaxZoomDistance},Callback=function(Value)
	me.CameraMaxZoomDistance = Value;
end});
Tabs.Player:Slider({Title="跳跃高度",Desc="调整跳跃高度",Value={Min=7.1,Max=25,Default=7.1},Callback=function(Value)
...
Tabs.Player:Toggle({Title="无限体力",Desc="字面意思",Value=functions.Infstamina,Callback=function(Value)
...
Tabs.Player:Dropdown({Title="无限体力方法",Desc="选择实现方法",Values={"Getgc","low exploit"},Value=Methods.Infstamina,Multi=false,AllowNone=false,Callback=function(Value)
	Methods.Infstamina = Value;
end});
Tabs.Player:Toggle({Title="无坠落伤害",Desc="防止坠落伤害",Value=functions.Nofalldamage,Callback=function(Value)
...
Tabs.Player:Toggle({Title="穿墙",Desc="可以穿过墙壁",Value=functions.Noclip,Callback=function(Value)
	...
	elseif RUNS.Noclip then
		RUNS.Noclip:Disconnect();
		RUNS.Noclip = nil;
	end
end});
Tabs.Player:Toggle({Title="伪装倒地",Desc="伪装成倒地状态",Value=functions.FakeDown,Callback=function(Value)
...
Tabs.Player:Toggle({Title="停止颈部移动",Desc="停止角色颈部移动",Value=functions.Stopneckmove,Callback=function(Value)
	...
	if Value then
		...
		me.CharacterAdded:Connect(function(char)
			if (char and char:FindFirstChild("Humanoid")) then
				...
				repeat
					task.wait();
				until char and char:FindFirstChild("Humanoid") 
				if functions.Stopneckmove then
					char:SetAttribute("NoNeckMovement", true);
				end
			end
...
Tabs.Player:Toggle({Title="肢体不碎",Desc="防止肢体断裂",Value=functions.Unbreaklimbs,Callback=function(Value)
...
Tabs.Combat:Toggle({Title="近战光环",Desc="最好别乱用",Value=functions.Meleeaura,Callback=function(Value)
	...
	if Value then
		...
		local attachcd = 0.1;
		local AttachCD = {Fists=0.05,Knuckledusters=0.05,Nunchucks=0.05,Shiv=0.05,Bat=1,["Metal-Bat"]=1,Chainsaw=2.5,Balisong=0.05,Rambo=0.3,Shovel=3,Sledgehammer=2,Katana=0.1,Wrench=0.1,FireAxe=2.6};
		local function Attack(target)
			if not (target and target:FindFirstChild("Head")) then
				return;
			end
			...
			if ((tick() - AttachTick) >= attachcd) then
				...
				if TOOL then
					...
					local arg2 = {"🍞",tick(),TOOL,"2389ZFX34",result,true,Handle,target:FindFirstChild(part),target,me.Character.HumanoidRootPart.Position,target:FindFirstChild(part).Position};
				...
				if mychar then
					...
					if myhrp then
						for _, a in pairs(plrs:GetPlayers()) do
							...
							if ((myhrp.Position - hrp.Position).Magnitude >= SectionSettings.MeleeAura.Distance) then
								continue;
							end
							...
							if (SectionSettings.MeleeAura.CheckDowned and CharStats(a).Downed.Value) then
								continue;
							end
...
Tabs.Combat:Toggle({Title="显示动画",Desc="显示攻击动画",Value=SectionSettings.MeleeAura.ShowAnim,Callback=function(Value)
	SectionSettings.MeleeAura.ShowAnim = Value;
end});
Tabs.Visual:Toggle({Title="ESP",Desc="显示玩家轮廓",Value=functions.ESP,Callback=function(Value)
	functions.ESP = Value;
	if Value then
...
Tabs.Visual:Toggle({Title="高亮显示",Desc="高亮显示其他玩家",Value=SectionSettings.ESP.Highlight,Callback=function(Value)
	SectionSettings.ESP.Highlight = Value;
end});
Tabs.Visual:Toggle({Title="手臂特效",Desc="改变手臂材质",Value=functions.ArmsChams,Callback=function(Value)
...
local ESPSettings = {Enabled=true,TeamCheck=true,MaxDistance=200,FontSize=11,FadeOut={OnDistance=true,OnDeath=false,OnLeave=false},Options={Teamcheck=false,TeamcheckRGB=Color3.fromRGB(0, 255, 0),Friendcheck=true,FriendcheckRGB=Color3.fromRGB(0, 255, 0),Highlight=false,HighlightRGB=Color3.fromRGB(255, 0, 0)},Drawing={Chams={Enabled=true,Thermal=true,FillRGB=Color3.fromRGB(255, 140, 0),Fill_Transparency=100,OutlineRGB=Color3.fromRGB(255, 140, 0),Outline_Transparency=100,VisibleCheck=true},Names={Enabled=true,RGB=Color3.fromRGB(255, 255, 255)},Flags={Enabled=true},Distances={Enabled=true,Position="Text",RGB=Color3.fromRGB(255, 255, 255)},Weapons={Enabled=true,WeaponTextRGB=Color3.fromRGB(255, 140, 0),Outlined=false,Gradient=false,GradientRGB1=Color3.fromRGB(255, 255, 255),GradientRGB2=Color3.fromRGB(255, 140, 0)},Healthbar={Enabled=true,HealthText=true,Lerp=false,HealthTextRGB=Color3.fromRGB(255, 140, 0),Width=2.5,Gradient=true,GradientRGB1=Color3.fromRGB(200, 0, 0),GradientRGB2=Color3.fromRGB(60, 60, 125),GradientRGB3=Color3.fromRGB(255, 140, 0)},Boxes={Animate=true,RotationSpeed=300,Gradient=false,GradientRGB1=Color3.fromRGB(255, 140, 0),GradientRGB2=Color3.fromRGB(0, 0, 0),GradientFill=true,GradientFillRGB1=Color3.fromRGB(255, 140, 0),GradientFillRGB2=Color3.fromRGB(0, 0, 0),Filled={Enabled=true,Transparency=0.75,RGB=Color3.fromRGB(0, 0, 0)},Full={Enabled=true,RGB=Color3.fromRGB(255, 255, 255)},Corner={Enabled=true,RGB=Color3.fromRGB(255, 140, 0)}}}};
local ESPManager = {};
do
	...
	ESPManager.Start = function(self)
		if ScreenGui then
			return;
		end
	...
	ESPManager.SetEnabled = function(self, enabled)
...
Tabs.Visual:Toggle({Title="ESP2",Desc="增强型玩家透视",Value=false,Callback=function(value)
	...
	for _, part in ipairs(me.Character:GetDescendants()) do
		if part:IsA("BasePart") then
			if not originalCharacterData[part] then
				originalCharacterData[part] = {material=part.Material,color=part.Color,transparency=part.Transparency};
			end
...
transparentToggle = Tabs.Visual:Toggle({Title="人物透明",Desc="使自己的角色半透明",Value=false,Callback=function(value)
	if value then
...
transparentRainbowToggle = Tabs.Visual:Toggle({Title="人物变色",Desc="角色颜色渐变",Value=false,Callback=function(value)
...
local Utility = {};
Utility.GetCharacter = function(self, plr)
	return plr.Character or plr.CharacterAdded:Wait();
end;
Utility.GetHumanoid = function(self, char)
	return char:FindFirstChildOfClass("Humanoid");
end;
Utility.GetHead = function(self, char)
	return char:FindFirstChild("Head");
end;
Utility.GetRootPart = function(self, char)
	return char:FindFirstChild("HumanoidRootPart");
end;
Utility.GetDistance = function(self, pos1, pos2)
	return (pos1 - pos2).Magnitude;
end;
Utility.Random = function(self, min, max)
	return math.random(min, max);
end;
local features = {HeadDamageReduction=false,HeadProtectionBox=false,HideHeadMode=false,AutoParry=false,AutoExecute=false,AutoParryDistance=10};
...
local function applyHideHead(character, hide)
	...
	if hide then
		originalHeadProperties[character] = {Transparency=head.Transparency,CanCollide=head.CanCollide,CanQuery=head.CanQuery,Massless=head.Massless,Size=head.Size};
		head.Transparency = 1;
...
local function toggleHeadDamageReduction(state)
	...
	elseif headDamageConnection then
		headDamageConnection:Disconnect();
		headDamageConnection = nil;
	end
end
...
MainTab:Button({Title="antikick(防踢 必须开)",Callback=function()
	local v4 = next;
...
MainTab:Toggle({Title="头部伤害减免",Value=false,Callback=toggleHeadDamageReduction});
MainTab:Toggle({Title="头部保护碰撞箱",Value=false,Callback=toggleHeadProtectionBox});
MainTab:Toggle({Title="藏头",Value=false,Callback=toggleHideHeadMode});
MainTab:Toggle({Title="自动格挡",Value=false,Callback=toggleAutoParry});
MainTab:Slider({Title="自动格挡距离",Value={Min=0,Max=30,Default=10},Callback=function(value)
	features.AutoParryDistance = value;
end});
MainTab:Toggle({Title="自动处决",Value=false,Callback=toggleAutoExecute});
